...
def Voxelization(w, h, intrinsic_matrix, extrinsic_matrix, point_clouds, valid_depth_near, valid_depth_far, num_planes):
    ...
    depth_tmp = np.linspace(valid_d_max, valid_d_min, num_planes+1)
    tmp = []
    # delta = np.zeros()

    for bin in range(num_planes):
        tmp.append(random.uniform(depth_tmp[bin],depth_tmp[bin + 1]))
    tmp = np.array(tmp, dtype=np.float32)
    print(tmp.shape)

    delta = np.zeros_like(tmp)
    for bin in range(num_planes - 1):
        delta[bin] = tmp[bin] - tmp[bin+1]
    ...
    return np.uint16(u_sorted_1), np.uint16(v_sorted_1), np.uint8(d_position_sorted_1), np.uint32(valid_position_sorted_1), \
           ...
           np.float16(distance_sorted_1),np.float32(delta)
...
if __name__ == '__main__':
    ...
    scene = 'scene0017_00'
    # dir1 = '../data/ScanNet_with_delta_11_copy/%s/depth/' % scene
    dir2 = 'D:/mycode/Neural-Point-Cloud-Rendering-via-Multi-Plane-Projection/data/ScanNet/%s/intrinsic/' % scene
    dir3 = 'D:/mycode/Neural-Point-Cloud-Rendering-via-Multi-Plane-Projection/data/ScanNet/%s/pose/' % scene
    dir4 = 'D:/mycode/Neural-Point-Cloud-Rendering-via-Multi-Plane-Projection/data/ScanNet/%s/color/' % scene
    point_clouds_path = 'D:/mycode/MPP/pre_processing_results/ScanNet_with_delta_1.5M_random_sampling/%s/point_clouds_simplified_1.5M.ply' % scene
    output_dir = 'D:/mycode/MPP/pre_processing_results/ScanNet_with_delta_1.5M_random_sampling/%s/reproject_results_%s/' % (scene, num)
    output_dir1 = 'D:/mycode/MPP/pre_processing_results/ScanNet_with_delta_1.5M_random_sampling/%s/weight_%s/' % (scene, num)
...
######################################################################################################################
    ...
    for i in range(len(color_names)):
        ...
        distance,delta = Voxelization(target_w, target_h, intrinsic_matrix, extrinsic_matrix, point_clouds, near, far, num)
        ...
        np.savez_compressed(output_dir + '%s_compressed' % i, u=u, v=v, d=d, select_index=index,
                            ...
                            distance=distance, each_split_max_num=split_each_max,delta=delta)
