...
def PositionalEncoding(view_direction,L):
    # 输出为 3
    # 输入为 3 * 2 * L


    PE = np.ones([view_direction.shape[0], view_direction.shape[1] * 2 * L])
    ...
    for i in range(3 * 2 * L):
        ...
        if i < (2 * L):
            ...
            if i % 2 == 0:
                PE[:, i] = np.sin((np.power(0.5, np.int(i / 2)) * math.pi * view_direction[:, np.int(0)]))

            else:
                PE[:, i] = np.cos((np.power(0.5, np.int((i - 1) / 2)) * math.pi * view_direction[:, np.int(0)]))

        if (2 * L) <= i < (4 * L):

            if i % 2 == 0:
                PE[:, i] = np.sin((np.power(0.5, np.int((i - (2 * L)) / 2)) * math.pi * view_direction[:, np.int(1)]))

            else:
                PE[:, i] = np.cos((np.power(0.5, np.int((i - (2 * L) - 1) / 2)) * math.pi * view_direction[:, np.int(1)]))

        if (4 * L) <= i < (6 * L):

            if i % 2 == 0:
                PE[:, i] = np.sin((np.power(0.5, np.int((i - (4 * L)) / 2)) * math.pi * view_direction[:, np.int(2)]))

            else:
                PE[:, i] = np.cos((np.power(0.5, np.int((i - (4 * L) - 1) / 2)) * math.pi * view_direction[:, np.int(2)]))

    return PE

def RGBEncoding(point_position,L):

    # 输入为 [3 , Nums_point]
    # 输出为 [3*2*L,N]

    point_position = np.transpose(point_position)  #维度为[N,3]
    PE = np.ones([point_position.shape[0],point_position.shape[1]*2*L])
    ...
    for i in range(3 * 2 * L):
        ...
        if i < (2 * L):
            ...
            if i % 2 == 0:
                PE[:, i] = np.sin((np.power(0.5, np.int(i / 2)) * math.pi * point_position[:, np.int(0)]))

            else:
                PE[:, i] = np.cos((np.power(0.5, np.int((i - 1) / 2)) * math.pi * point_position[:, np.int(0)]))

        if (2 * L) <= i < (4 * L):

            if i % 2 == 0:
                PE[:, i] = np.sin((np.power(0.5, np.int((i-(2*L)) / 2)) * math.pi * point_position[:, np.int(1)]))

            else:
                PE[:, i] = np.cos((np.power(0.5, np.int((i-(2*L)-1) / 2)) * math.pi * point_position[:, np.int(1)]))

        if (4 * L) <= i < (6 * L):

            if i % 2 == 0:
                PE[:, i] = np.sin((np.power(0.5, np.int((i-(4*L)) / 2)) * math.pi * point_position[:, np.int(2)]))

            else:
                PE[:, i] = np.cos((np.power(0.5, np.int((i-(4*L)-1) / 2)) * math.pi * point_position[:, np.int(2)]))

    return np.transpose(PE)
