...
pre_process_data = 'ScanNet_with_delta_11_random_sampling'
scene = 'scene0010_00'  # scene name
task = '%s_npcr_%s_random_sampling_gpu00_0_34_lossv1' % (dataset, scene)  # task name, also path of checkpoints file
...
dir5 = 'pre_processing_results/ScanNet_with_delta_11_random_sampling/%s/point_clouds_simplified_6.ply' % (scene)  # point clouds file path

num_image = len(glob.glob(os.path.join(dir1, '*.jpg')))

image_names_train, index_names_train, camera_names_train, index_names_1_train,\
image_names_test, index_names_test, camera_names_test, index_names_1_test,\
    camera_names, index_names, index_names_1 = prepare_data_ScanNet(dir1, dir2, dir3, dir4, num_image)
...
if is_training:
    ...
    for epoch in range(num_epoch):
        ...
        # for i in range(4):
            ...
            if not (os.path.isfile(camera_name) and os.path.isfile(image_name) and os.path.isfile(index_name) and os.path.isfile(index_name_1)):
                print("Missing file!")
                if not os.path.isfile(camera_name):
                    print(camera_name)
                if not os.path.isfile(image_name):
                    print(image_name)
                if not os.path.isfile(index_name):
                    print(index_name)
                if not os.path.isfile(index_name_1):
                    print(index_name_1)
                continue
    ...
    output_path = "%s/Test_Result_crop_only/" % (task)
    if not os.path.isdir(output_path):
        os.makedirs(output_path)
    ...
    for id in range(len(camera_names)):
        ...
        cv2.imwrite(output_path + '%d.png' % id, np.uint8(result[0, :, :, :]))
